...
def permutation(x, max_segments=5, seg_mode="equal"):
    ...
    num_segs = np.random.randint(1, max_segments, size=(x.shape[0]))
    numbers_set = [1, 2, 4, 6, 8]
    num_segs = np.random.choice(numbers_set, size=len(num_segs), replace=True)
    # print('num_segs:', num_segs, len(num_segs))
    ret = np.zeros_like(x)
    for i, pat in enumerate(x):
        if num_segs[i] > 1:
            if x.shape[1] % num_segs[i] >0: break
            if seg_mode == "random":
                ...
                splits = np.array_split(orig_steps, num_segs[i])
            # print('splits:',splits, splits[0].shape, splits[1].shape, splits[-1].shape, len(splits), max_segments)
            warp = np.concatenate(np.random.permutation(splits)).ravel()
...
def DataTransform(sample, config):
    ...
    weak_aug = scaling(sample, sigma=config.jitter_scale_ratio)
    strong_aug = jitter(permutation(sample, max_segments=config.max_seg), config.jitter_ratio)
    # print("config.jitter_ratio:",config.jitter_ratio)
    return weak_aug, strong_aug

def DataTransform_FD(sample, config):
    pertub_ratio = config.mix_rate
    # print("pertub_ratio111:",pertub_ratio)
...
def remove_frequency(x, pertub_ratio=0.0):
    mask = torch.empty(x.shape, device=x.device).uniform_() > pertub_ratio # maskout_ratio are False
...
def add_frequency(x, pertub_ratio=0.0):
    mask = torch.empty(x.shape, device=x.device).uniform_() > (1-pertub_ratio) # only pertub_ratio of all values are True
